...
def floatTypeData(col_values):
    ...
    try:
        ...
        mean_after_float = np.mean([len(str(float(val)).split('.')[1]) for val in float_type_data if
                                    len(str(float(val)).split('.')) > 1 and pd.notnull(val)])
        max_after_float = max([int(str(float(val)).split('.')[1]) for val in float_type_data if
                               len(str(float(val)).split('.')) > 1 and pd.notnull(val)])
    except:
        mean_before_float = 0
        mean_after_float = 0
...
def additional_features(col_values: list, date_samples=1000):
    ...
    if len(float_type) > 0:
        float_mean, float_variance, float_skew, float_kurtosis, float_min, float_max, float_sum = compute_stats(
            float_type)
    ...
    if len(word_len_data) > 0:
        wordlen_mean, wordlen_variance, wordlen_skew, wordlen_kurtosis, wordlen_min, wordlen_max, wordlen_sum = compute_stats(
            word_len_data)
        wordlen_median = np.median(word_len_data)

    print('Custom feature creation case based features:', datetime.now())

    return [total_vals, alphaNumRatio, numericRatio, alphaRatio, otherRatio, dateRatio, dayRatio, intRatio, floatRatio,
            rangeRatio,
            int_mean, int_variance, int_skew, int_kurtosis, int_min, int_max, int_sum, int_median, mean_before_int,
            float_mean, float_variance,
            float_skew, float_kurtosis, float_min, float_max, float_sum, float_median, zero_flag, mean_before_float,
            mean_after_float,
            max_after_float, mean_uppercase, mean_lowercase, wordlen_mean, wordlen_variance, wordlen_skew,
            wordlen_kurtosis, wordlen_min,
            wordlen_max, wordlen_sum, wordlen_median]


def extract_addl_feats(col_values: list, features: OrderedDict):
    feats_name = ['total_vals', 'alphaNumRatio', 'numericRatio', 'alphaRatio', 'otherRatio', 'dateRatio', 'dayRatio',
                  'intRatio', 'floatRatio',
                  'rangeRatio', 'int_mean', 'int_variance', 'int_skew', 'int_kurtosis', 'int_min', 'int_max', 'int_sum',
                  'int_median', 'mean_before_int',
                  'float_mean', 'float_variance', 'float_skew', 'float_kurtosis', 'float_min', 'float_max', 'float_sum',
                  'float_median', 'zero_flag',
                  'mean_before_float', 'mean_after_float', 'max_after_float', 'mean_uppercase', 'mean_lowercase',
                  'wordlen_mean', 'wordlen_variance',
